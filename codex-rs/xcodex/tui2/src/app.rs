...
impl App {
    ...
    fn training_mode_prompt(request: &TrainingModeRequest) -> Result<String, String> {
    ...
    async fn generate_training_mode_artifact(
        &self,
        request: &TrainingModeRequest,
    ) -> Result<TrainingModeArtifact, String> {
        ...
        helper_config.model = Some(self.current_model.clone());
    ...
    fn write_training_mode_payload(payload: &TrainingModePayload) -> Result<PathBuf, String> {
    ...
    ) -> Result<TrainingModeResult, String> {
    ...
    async fn handle_open_manual_patch_apply(
        ...
        let paths: Vec<String> = request
            ...
            .collect();
        if let Some(thread_id) = self.effective_conversation_id()
            && let Err(err) = self
                .server
    ...
    async fn handle_open_training_mode(
        ...
        let payload =
            Self::training_mode_payload(&request, self.effective_conversation_id(), training);
            ...
            .await
        {
            Ok(()) => match Self::read_training_mode_result(&result_path, &approval_id) {
                Ok(result) => match result.status {
                    ...
                    TrainingModeStatus::Cancelled => {
                        self.chat_widget.add_info_message(
                            "Training mode was cancelled. The turn was interrupted.".to_string(),
                            Some(format!("Result file: {}", result_path.display())),
                        );
                        ReviewDecision::Abort
                    }
            ...
            Err(err) => {
                self.chat_widget
                    .add_error_message(format!("{err} Training mode was not completed."));
                ReviewDecision::Abort
            }
        };
